...
public class Terrain {
	...
	//modulo Jeu X et Y (fonction rendant la coordonée modulo Jeu) (
	// à simplifier dans inv
	...
	public boolean EstChutable(int x, int y) {
		if( (PrendreBloc(x,y).PrendreType() == TypeBloc.DIAMANT) || (PrendreBloc(x,y).PrendreType() == TypeBloc.ROCHER)) {
			int ye =  EstModuloY(y+1);
			if( (PrendreBloc(x,ye).PrendreType() == TypeBloc.VIDE)) {
				return true;
			}
		}
		return false;
	...
	public void Chutes(int x, int y) {
		...
		if(EstChutable(x,y)) {
			PrendreBloc(x,ye).MettreType(PrendreBloc(x,y).PrendreType());
